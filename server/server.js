...
// Publish + AI (Groq, with images + math) + Paynow + site serving
...
const AI_FREE_LIMIT = 250;      // messages per day for free users
const AI_IMAGE_LIMIT = 50;      // image requests per day for free users
...
// ============================================================
// AI CHAT (Groq) — images + math + video links
// ============================================================
  ...
  try {
    ...
    if (wantsImage && plan === 'free'){
      ...
      if ((imgCount || 0) >= AI_IMAGE_LIMIT){
        return res.status(402).json({
          error: `Free plan includes ${AI_IMAGE_LIMIT} AI images per day. Upgrade to Pro for unlimited.`,
          upgrade: true,
          reason: 'images'
        });
      }
    ...
    if (wantsImage && lastUser){
      await db.from('ai_images').insert({
        user_id: userId,
        prompt: lastUser.content.slice(0, 400)
      });
    }
    ...
    const systemPrompt = [
      ...
      'Users are building websites on their phones. Help them write HTML, CSS, and JavaScript.',
      'Assume beginner level unless the user is clearly advanced. Never invent KUMG features that do not exist.',
      '',
      'STYLE RULES:',
      '- Keep answers short: 2-3 short paragraphs, then code.',
      '- Wrap code in triple-backtick fences with a language tag: ```html, ```css, ```js.',
      '',
      'MATH RULES (very important):',
      '- When the user asks for a calculation, show it like Google does: clean, one line per step.',
      '- Never use $ signs for math. Never use "USD" unless they asked about money.',
      '- Use standard math symbols: × ÷ + − = and the % sign for percent.',
      '- Example: for "what is 15% of 200?", respond exactly like:',
      '  15% of 200',
      '  = 15 ÷ 100 × 200',
      '  = 0.15 × 200',
      '  = 30',
      '- Round money values to 2 decimals only when the user asks about money.',
      '',
      'IMAGE RULES:',
      '- When the user asks for an image, a picture, a photo, or an illustration, respond with ONE markdown image line:',
      '  ![short description](https://image.pollinations.ai/prompt/URL_ENCODED_PROMPT?width=1024&height=768&nologo=true)',
      '  Replace URL_ENCODED_PROMPT with words joined by + (no spaces). Example: a+green+hexagon+on+black+background',
      '- You may also include 1 short sentence after the image.',
      '- Never make up other image URLs.',
      '',
      'VIDEO RULES:',
      '- When the user asks for a video, respond with a Pexels search link on its own line:',
      '  🎥 [Watch videos about TOPIC](https://www.pexels.com/search/videos/TOPIC/)',
      '  Replace TOPIC with words joined by %20 (URL encoded).',
      '- Never claim to embed videos directly.'
    ].join('\n');

    const contextNote = projectContext
      ? '\n\nCurrent project context (for reference — do not repeat back):\n' +
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('Free limits — messages: ' + AI_FREE_LIMIT + ', images: ' + AI_IMAGE_LIMIT);
});
