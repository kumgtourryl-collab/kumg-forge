...
function renderKeys(){
  ...
  ckScrollEl.querySelectorAll('.ck-key').forEach(btn => {
    // pointerdown prevents the textarea from losing focus
...
function insertAtCursor(text){
  ...
  // Caret placement — smart for paired inserts
...
// Show when textarea has focus, hide when it loses focus
editorEl.addEventListener('focus', () => showKeys(true));
editorEl.addEventListener('blur', () => {
  // small delay so tapping a key doesn't hide the bar
  setTimeout(() => {
    if (document.activeElement !== editorEl && !document.activeElement?.classList?.contains('ck-key')){
      showKeys(false);
    }
  }, 180);
});

// Keep the bar glued above the Android keyboard using visualViewport
if (window.visualViewport){
  const vv = window.visualViewport;
  function positionKeys(){
    const kb = Math.max(0, window.innerHeight - vv.height - vv.offsetTop);
    codeKeysEl.style.bottom = kb + 'px';
  }
  vv.addEventListener('resize', positionKeys);
  vv.addEventListener('scroll', positionKeys);
  positionKeys();
}
